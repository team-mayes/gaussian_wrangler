...
INPUT_BASENAME=ethylrad_opt_stable
INPUT_FILE=tests/test_data/run_gauss/stable.tpl
GAUSSIAN_EXEC=g16
MEMSIZE=5GB
SCRATCH=/scratch/hmayes/ethylrad_opt_stable_${SLURM_JOB_ID}
SCRATCH2=/dev/shm
...
if [ ${NUMRWFLINES} -eq 1 ]; then
    ...
    echo "%NoSave" >> ${INFILE}
    echo "%OldChk=${SCRATCH2}/ethylrad_opt.chk" >> ${INFILE}
    echo "%Chk=${SCRATCH2}/ethylrad_opt_stable.chk" >> ${INFILE}
